        ...
        <header class="h-16 border-b border-white/10 bg-black/80 backdrop-blur-xl flex items-center px-6 sticky top-0 z-50 transition-all duration-300">
            ...
            <nav class="flex items-center gap-1 relative z-50">
                <button v-for="tab in tabs" :key="tab.id"
                    ...
                    :class="activeTab === tab.id ? 'text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-white/5'">
                    ...
                    <!-- Active Glow Effect -->
                    <div v-if="activeTab === tab.id" class="absolute inset-0 bg-white/5 border border-white/10 rounded-lg shadow-[0_0_15px_rgba(255,255,255,0.05)]"></div>
                    <div v-if="activeTab === tab.id" class="absolute bottom-0 left-1/2 -translate-x-1/2 w-1/2 h-[2px] bg-amber-500 shadow-[0_0_8px_#f59e0b]"></div>
                </button>
            </nav>
        ...
        <div class="md:hidden fixed bottom-0 left-0 right-0 border-t border-white/10 bg-black/90 backdrop-blur-md pb-safe">
            <div class="flex justify-around p-2">
                 <button v-for="tab in mobileTabs" :key="tab.id"
                    ...
                    :class="activeTab === tab.id ? 'text-white' : 'text-slate-600'">
...
const tabs = [
    ...
    { id: 'tenant', label: 'Tenants', icon: '🏢' },
    { id: 'permissions', label: 'Access', icon: '🔐' },
    { id: 'system', label: 'System', icon: '⚙️' },
];

const filteredTabs = computed(() => tabs);
// Mobile shows fewer tabs + Menu
const mobileTabs = computed(() => tabs.slice(0, 5));
