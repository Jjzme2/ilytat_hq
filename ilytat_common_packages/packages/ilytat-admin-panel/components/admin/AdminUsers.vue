...
const performUserAction = async (uid: string, email: string, action: 'send-reset-email' | 'force-reset' | 'clear-reset-flag') => {
    ...
    try {
        if (action === 'send-reset-email') {
             const link = await adminAdapter.resetUserPassword(uid)
             success(`Reset email sent to ${email}`)
             if (link) console.log('Reset Link:', link)
        }
    } catch (e: any) {
...
<template>
    <div class="space-y-6 animate-fade-in">
            ...
            <table class="w-full text-left max-w-full">
                <thead>
                    <tr class="border-b border-white/5 bg-white/[0.02]">
                        ...
                        <th class="px-6 py-3 text-[11px] font-medium text-slate-500 uppercase tracking-wider text-center">ID</th>
                        <th class="px-6 py-3 text-[11px] font-medium text-slate-500 uppercase tracking-wider">Role</th>
                        <th class="px-6 py-3 text-[11px] font-medium text-slate-500 uppercase tracking-wider">Tenant</th>
                        <th class="px-6 py-3 text-[11px] font-medium text-slate-500 uppercase tracking-wider">Status</th>
                        <th class="px-6 py-3 text-[11px] font-medium text-slate-500 uppercase tracking-wider">Last Active</th>
                        <th class="px-6 py-3 text-[11px] font-medium text-slate-500 uppercase tracking-wider text-right">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr v-else v-for="user in users" :key="user.uid" class="group hover:bg-white/[0.02] transition-colors">
                        <td class="px-6 py-3">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-xs font-bold text-slate-300">
                                    {{ user.email?.[0]?.toUpperCase() }}
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-sm font-medium text-slate-200">{{ user.displayName || user.email }}</span>
                                    <span class="text-[10px] text-slate-600 font-mono">{{ user.uid.substring(0, 8) }}</span>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-3">
                             <span v-if="user.disabled" class="text-[10px] text-rose-500 bg-rose-500/10 px-1.5 py-0.5 rounded">Disabled</span>
                             <span v-else class="text-[10px] text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded">Active</span>
                        </td>
                        ...
                        <td class="px-6 py-3 text-right">
                            <div class="flex justify-end items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button @click="openMessageModal(user)" 
                                    class="p-1.5 text-slate-500 hover:text-rose-400 rounded hover:bg-white/10" title="Send Message">
                                    💬
                                </button>
                                <button @click="openEditModal(user)" 
                                    class="p-1.5 text-slate-500 hover:text-white rounded hover:bg-white/10" title="Edit Professional Info">
                                    📝
                                </button>
                                <button @click="performUserAction(user.uid, user.email, 'send-reset-email')" 
                                    class="p-1.5 text-slate-500 hover:text-amber-400 rounded hover:bg-white/10" title="Send Password Reset">
                                    🔑
                                </button>
        ...
        <Teleport to="body">
            <div v-if="showTenantModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
                <div class="absolute inset-0 bg-black/80" @click="showTenantModal = false"></div>
                <div class="relative bg-zinc-900 border border-white/10 w-full max-w-sm rounded-xl shadow-2xl p-6 space-y-4">
                    <h3 class="text-lg font-bold text-white">Assign Tenant</h3>
                    <p class="text-sm text-slate-400">Assign {{ selectedUser?.email }} to:</p>
                    <input v-model="newTenantId" placeholder="Tenant ID..." class="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-white" />
                    <div class="flex justify-end gap-2 pt-2">
                        <button @click="showTenantModal = false" class="px-3 py-2 text-sm text-slate-400 hover:text-white">Cancel</button>
                        <button @click="saveTenant" class="px-4 py-2 bg-white text-black font-bold rounded-lg text-sm hover:bg-slate-200 transition-colors">Save</button>
                    </div>
                </div>
            </div>
        </Teleport>
        ...
        <!-- Messaging Modal -->
        <Teleport to="body">
            <div v-if="showMessageModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
                <div class="absolute inset-0 bg-black/80" @click="showMessageModal = false"></div>
                <div class="relative bg-zinc-900 border border-white/10 w-full max-w-md rounded-xl shadow-2xl p-6 space-y-4">
                    ...
                    <div class="flex justify-end gap-2 pt-2">
                        <button @click="showMessageModal = false" class="px-3 py-2 text-sm text-slate-400 hover:text-white">Cancel</button>
        ...
        <!-- Edit Profile Modal -->
        <Teleport to="body">
             <div v-if="showEditModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
                ...
                <div class="relative bg-zinc-900 border border-white/10 w-full max-w-sm rounded-xl shadow-2xl p-6 space-y-4">
                    ...
                    <div class="space-y-4">
                         <div>
                            <label class="text-[10px] text-slate-500 uppercase font-bold mb-1 block">Display Name</label>
                            <input v-model="editData.displayName" class="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-white" />
                        </div>
                        <div>
                            <label class="text-[10px] text-slate-500 uppercase font-bold mb-1 block">Employee ID</label>
                            <input v-model="editData.employeeId" type="number" class="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-white font-mono" />
                        </div>
                    </div>
                    <div class="flex justify-end gap-2 pt-2">
                        <button @click="showEditModal = false" class="px-3 py-2 text-sm text-slate-400 hover:text-white">Cancel</button>
                        <button @click="saveUserEdit" class="px-4 py-2 bg-blue-500 text-white font-bold rounded-lg text-sm hover:bg-blue-600 transition-colors">Update Profile</button>
                    </div>
                </div>
            </div>
        </Teleport>
