...
describe('useCommandPalette', () => {
    ...
    it('registerCommands adds multiple commands and prevents duplicates', () => {
        ...
        const cmd1 = { id: 'c1', label: 'Command 1', action: () => { } }
        const cmd2 = { id: 'c2', label: 'Command 2', action: () => { } }
        const cmd3 = { id: 'c1', label: 'Command 1 Duplicate', action: () => { } }
        registerCommands([cmd1, cmd2, cmd3])
        expect(commands.value.length).toBe(2)
        expect(commands.value[0].id).toBe('c1')
        expect(commands.value[1].id).toBe('c2')
    })
