...
describe('useCommandPalette', () => {
    ...
    it('registerCommands prevents duplicates in batch', () => {
        const { registerCommands, commands } = useCommandPalette()
        registerCommands([
            { id: '1', label: 'One', action: () => { } },
    ...
    it('registerCommands prevents duplicates against existing', () => {
        const { registerCommand, registerCommands, commands } = useCommandPalette()
        registerCommand({ id: '1', label: 'Existing', action: () => { } })

        registerCommands([
            { id: '1', label: 'Duplicate', action: () => { } },
            { id: '2', label: 'New', action: () => { } }
        ])
        expect(commands.value).toHaveLength(2)
        expect(commands.value[0].label).toBe('Existing')
        expect(commands.value[1].label).toBe('New')
    })
