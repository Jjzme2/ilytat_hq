<template>
    <div class="h-full flex flex-col">
        <!-- Loading -->
        <!-- Loading -->
        <div v-if="projectLoading" class="p-6 space-y-8 animate-pulse">
            <div class="space-y-4">
                <SkeletonLoader width="200px" height="20px" class="mb-4" />
                <SkeletonLoader width="40%" height="40px" />
                <div class="flex gap-4 mt-6">
                    <SkeletonLoader width="100px" height="32px" :rounded="true" />
                    <SkeletonLoader width="100px" height="32px" :rounded="true" />
                    <SkeletonLoader width="100px" height="32px" :rounded="true" />
                </div>
            </div>
            <div class="border-b border-white/5 my-6"></div>
        ...
        <div v-else-if="projectError" class="p-6">
            <div class="p-4 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400">
                {{ projectError }}
                <NuxtLink to="/projects" class="block mt-2 text-sm text-red-300 hover:text-red-200 underline">
                    Return to projects
                </NuxtLink>
            </div>
        </div>
        ...
        <template v-else-if="currentProject">
            <!-- Header -->
            <header
                class="flex-none px-4 md:px-6 py-4 md:py-6 border-b border-white/10 bg-zinc-900/50 backdrop-blur-sm">
                ...
                <nav class="flex gap-1 -mb-px min-w-max">
                    <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id" :class="[
                        ...
                        <span v-if="tab.count > 0"
                            class="ml-1.5 px-1.5 py-0.5 text-[10px] rounded-full bg-zinc-800 text-zinc-400">{{ tab.count
                            }}</span>
                    </button>
                </nav>
            </div>
            ...
            <main class="flex-1 overflow-y-auto p-3 md:p-6 scrollbar-thin">
                <div class="max-w-4xl space-y-6 md:space-y-8">
                    ...
                    <div v-if="activeTab === 'overview'">
                        ...
                        <section>
                            ...
                            <div class="p-6 bg-zinc-900/40 border border-white/5 rounded-2xl">
                                <p class="text-zinc-300 leading-relaxed whitespace-pre-wrap">{{
                                    currentProject.description || 'No description provided.' }}</p>
                            </div>
                        </section>
                        ...
                        <section>
                            ...
                            <div class="h-4 bg-zinc-900 rounded-full border border-white/5 overflow-hidden">
                                <div class="h-full bg-gradient-to-r from-blue-600 to-cyan-500 transition-all duration-500"
                        ...
                        <section class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="p-5 bg-zinc-900/40 border border-white/5 rounded-xl">
                                ...
                                <p class="text-lg font-medium text-white">{{ currentProject.startDate ?
                                    formatDate(currentProject.startDate) : 'Not set' }}</p>
                            </div>
                            <div class="p-5 bg-zinc-900/40 border border-white/5 rounded-xl">
                    ...
                    <div v-else-if="activeTab === 'goals'">
                        ...
                        <form v-if="showGoalForm" @submit.prevent="handleCreateGoal"
                            class="mb-6 p-4 bg-zinc-900/60 border border-white/10 rounded-xl space-y-3">
                            <input v-model="newGoalTitle" type="text" required placeholder="Goal title..."
                            ...
                            <textarea v-model="newGoalDesc" rows="2" placeholder="Description (optional)..."
                                class="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500"></textarea>
                            <div class="flex justify-end">
                                <button type="submit" :disabled="goalsLoading"
                                    class="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-medium disabled:opacity-50">
                        ...
                        <!-- Loading -->
                        <!-- Loading -->
                        <div v-if="goalsLoading && goals.length === 0" class="space-y-4">
                            ...
                            <div v-for="goal in goals" :key="goal.id"
                                ...
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        ...
                                        <p v-if="goal.description" class="text-sm text-zinc-400 mt-1 line-clamp-2">{{
                                            goal.description }}</p>
                                    </div>
                    ...
                    <div v-else-if="activeTab === 'tasks'">
                        ...
                        <form v-if="showTaskForm" @submit.prevent="handleCreateTask()"
                            class="mb-6 p-4 bg-zinc-900/60 border border-white/10 rounded-xl space-y-3">
                            <input v-model="newTaskTitle" type="text" required placeholder="Task title..."
                                class="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500" />
                            <div class="grid grid-cols-2 gap-3">
                                <select v-model="newTaskPriority"
                                    class="bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500">
                                    <option :value="Priority.LOW">Low</option>
                                    <option :value="Priority.MEDIUM">Medium</option>
                                    <option :value="Priority.HIGH">High</option>
                                    <option :value="Priority.CRITICAL">Critical</option>
                                </select>
                                <select v-model="newTaskGoalId"
                                    class="bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500">
                        ...
                        <!-- Loading -->
                        <!-- Loading -->
                        <div v-if="tasksLoading && tasks.length === 0" class="space-y-4">
                                ...
                                <div
                                    ...
                                    <div class="flex items-start justify-between">
                                        <div class="flex items-start gap-3 flex-1">
                                            <button @click="handleToggleTask(task)" :class="[
                                            ...
                                            <div class="flex-1">
                                                <h3
                                                    :class="['font-medium', task.isCompleted ? 'text-zinc-500 line-through' : 'text-white']">
                                                    {{ task.title }}</h3>
                                                <div class="flex items-center gap-2 mt-1">
                                                    <span
                                                        :class="['text-[10px] px-1.5 py-0.5 rounded border', task.priorityColor]">{{
                                                            capitalize(task.priority) }}</span>
                                                    <span
                                                        :class="['text-[10px] px-1.5 py-0.5 rounded border', task.statusColor]">{{
                                                            task.formattedStatus }}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-2 ml-4">
                                    ...
                                    <!-- Subtask inline form -->
                                    <form v-if="subtaskFormParentId === task.id"
                                        @submit.prevent="handleCreateSubtask(task.id)" class="mt-3 flex gap-2">
                                        <input v-model="newSubtaskTitle" type="text" required
                                        ...
                                        <button type="submit" :disabled="tasksLoading"
                                            class="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded text-xs font-medium disabled:opacity-50">
                                            Add
                                        </button>
                                    </form>

                                    <!-- Subtasks -->
                                    <div v-if="getSubtasks(task.id).length > 0"
                                        class="mt-3 ml-8 space-y-2 border-l-2 border-zinc-800 pl-3">
                                        <div v-for="sub in getSubtasks(task.id)" :key="sub.id"
                                            class="group/sub flex items-center gap-2">
                                            <button @click="handleToggleTask(sub)" :class="[
                    ...
                    <div v-else-if="activeTab === 'notes'">
                        ...
                        <form v-if="showNoteForm" @submit.prevent="handleCreateNote"
                            class="mb-6 p-4 bg-zinc-900/60 border border-white/10 rounded-xl space-y-3">
                            <input v-model="newNoteTitle" type="text" required placeholder="Note title..."
                                class="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500" />
                            <textarea v-model="newNoteContent" rows="4" placeholder="Write your note..."
                                class="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500"></textarea>
                            <div class="flex justify-end">
                        ...
                        <!-- Loading -->
                        <!-- Loading -->
                        <div v-if="notesLoading && notes.length === 0" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <SkeletonLoader v-for="i in 2" :key="i" height="120px" />
                        </div>
                    ...
                    <div v-else-if="activeTab === 'links'">
                        <div class="flex items-center justify-between mb-4">
                            <h2 class="text-sm font-semibold text-zinc-500 uppercase tracking-wider">Quick Launch Links
                            </h2>
                        ...
                        <form v-if="showLinkForm" @submit.prevent="handleCreateLink"
                            class="mb-6 p-4 bg-zinc-900/60 border border-white/10 rounded-xl space-y-3">
                            <input v-model="newLinkLabel" type="text" required
                                placeholder="Link label (e.g. Firebase Console)..."
                                class="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500" />
                            <input v-model="newLinkUrl" type="url" required placeholder="https://..."
                                class="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500" />
                            <div class="flex justify-end">
                                <button type="submit" :disabled="quickLinksLoading"
                                    class="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-medium disabled:opacity-50">
                            ...
                            <div v-for="link in quickLinks" :key="link.id"
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <a :href="link.url" target="_blank" rel="noopener"
                                        class="text-xs text-blue-400 hover:text-blue-300 truncate block">{{ link.url
                                        }}</a>
                                </div>
                                <div class="flex items-center gap-2">
                                    <a :href="link.url" target="_blank" rel="noopener"
        ...
        <ClientOnly>
            <Dialog :open="isEditModalOpen" @close="isEditModalOpen = false" class="relative z-50">
                ...
                <div class="fixed inset-0 flex items-center justify-center p-4">
                    <DialogPanel v-if="currentProject"
                        class="w-full max-w-lg rounded-2xl bg-zinc-900 border border-white/10 p-6 shadow-xl">
...
const breadcrumbItems = computed(() => [
    ...
    { label: currentProject.value?.name || 'Loading...', icon: 'i-heroicons-document-text' }
]);
