<template>
    <div class="h-full flex flex-col">
        <!-- Header -->
        <header
            class="flex-none px-4 md:px-6 py-3 md:py-4 border-b border-white/10 flex flex-col gap-2 bg-zinc-900/50 backdrop-blur-sm sticky top-0 z-10">
            <div>
                <Breadcrumbs
                    :items="[{ label: 'Home', to: '/', icon: 'i-heroicons-home' }, { label: 'Documents', icon: 'i-heroicons-document-text' }]"
                    class="mb-2" />
                <h1
                    class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-purple-300">
                    Documents
                </h1>
                ...
                <div v-if="activeTab === 'documents'">
                    <div class="flex items-center justify-between mb-4 gap-4">
                        <div class="flex items-center gap-4 flex-1">
                            <h2 class="text-sm font-semibold text-zinc-500 uppercase tracking-wider whitespace-nowrap">
                                Documents</h2>
                            <!-- Search Bar -->
                    ...
                    <div v-if="showDocForm" @click.self="showDocForm = false"
                        class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-6">
                        <div
                            class="bg-zinc-900 border border-white/10 rounded-2xl w-full max-w-7xl h-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
                            ...
                            <div class="flex-1 overflow-hidden p-6">
                                <DocumentCreator v-if="!isEditMode" :initialTemplateId="selectedTemplateId"
                                    @save="handleFactorySave" @cancel="showDocForm = false" />

                                <!-- Simple Edit Form for existing docs (fallback) -->
                                <form v-else @submit.prevent="handleCreateOrUpdateDocument"
                                    class="max-w-xl mx-auto space-y-4">
                                    <input v-model="newDocTitle" type="text" required placeholder="Document title..."
                                    ...
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <button type="submit" :disabled="isLoading"
                                            class="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-medium disabled:opacity-50">
                                            Update Document
                                        </button>
                                    </div>
                                </form>
                            ...
                            <div class="flex items-start justify-between mb-3">
                                ...
                                <button @click.stop="openEditForm(doc)"
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 256 256"
                                        fill="currentColor">
                                        <path
                                            d="M227.31,73.37,182.63,28.68a16,16,0,0,0-22.63,0L36.69,152.05a16,16,0,0,0-4.69,11.31v44.69a16,16,0,0,0,16,16H92.69a16,16,0,0,0,11.31-4.69L227.31,96A16,16,0,0,0,227.31,73.37ZM92.69,208H48V163.31l88-88L180.69,120ZM192,108.68,147.31,64l24-24L216,84.68Z">
                                        </path>
                                    </svg>
                                </button>
                                <button @click.stop="handleDeleteDocument(doc.id)"
                ...
                <div v-else-if="activeTab === 'files'">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-3">
                            <h2 class="text-sm font-semibold text-zinc-500 uppercase tracking-wider">Files (R2 Storage)
                            </h2>
                    ...
                    <!-- Upload Progress -->
                    <div v-if="isLoadingR2 && uploadingFileName"
                        class="mb-4 p-4 bg-zinc-900/60 border border-white/10 rounded-xl">
                        <div class="flex items-center gap-3">
                            <div
                    ...
                    <!-- R2 Error -->
                    <div v-if="r2Error"
                        class="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-sm text-red-400">
                        {{ r2Error }}
                    </div>

                    <!-- Loading -->
                    <div v-if="isLoadingR2 && displayedFiles.length === 0 && !uploadingFileName"
                    ...
                    <div v-else-if="displayedFiles.length > 0 && viewMode === 'list'" class="space-y-2">
                        <div v-for="file in displayedFiles" :key="file.key"
                            ...
                            <div class="w-10 h-10 flex items-center justify-center rounded-lg bg-white/5 text-zinc-400">
                                ...
                                <svg v-else-if="getFileType(file.key) === 'image'" xmlns="http://www.w3.org/2000/svg"
                                    width="24" height="24" viewBox="0 0 256 256" fill="#c084fc">
                                    <path
                                        d="M216,40H40A16,16,0,0,0,24,56V200a16,16,0,0,0,16,16H216a16,16,0,0,0,16-16V56A16,16,0,0,0,216,40Zm0,160H40V56H216V200ZM76,96A12,12,0,1,1,88,84,12,12,0,0,1,76,96Zm124,88H56a8,8,0,0,1-5.66-13.66l48-48a8,8,0,0,1,11.32,0l40,40,34.34-34.34a8,8,0,0,1,11.32,0l26.34,26.34A8,8,0,0,1,200,184Z">
                                    </path>
                                </svg>
                                <svg v-else xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                    viewBox="0 0 256 256" fill="currentColor">
                                    <path
                                        d="M213.66,82.34l-56-56A8,8,0,0,0,152,24H56A16,16,0,0,0,40,40V216a16,16,0,0,0,16,16H200a16,16,0,0,0,16-16V88A8,8,0,0,0,213.66,82.34ZM160,51.31,188.69,80H160ZM200,216H56V40h96V88a8,8,0,0,0,8,8h48V216Z">
                                    </path>
                                </svg>
                            </div>
                            ...
                            <div class="flex items-center gap-2">
                                <button @click="handleViewFile(file.key)"
                                ...
                                <button @click="handleDownloadFile(file.key)"
                                ...
                                <button @click="handleDeleteR2File(file.key)"
                    ...
                    <div v-else-if="displayedFiles.length > 0"
                        ...
                        <div v-for="file in displayedFiles" :key="file.key"
                            ...
                            <div class="absolute inset-0 flex items-center justify-center p-4">
                                <img v-if="isImage(file.key)" :src="`/api/docs?key=${encodeURIComponent(file.key)}`"
                                    class="w-full h-full object-contain" loading="lazy" />
                                <div v-else class="text-zinc-500">
                                    <svg v-if="getFileType(file.key) === 'pdf'" xmlns="http://www.w3.org/2000/svg"
                                        width="64" height="64" viewBox="0 0 256 256" fill="#f87171">
                                        <path
                                            d="M224,152a8,8,0,0,1-8,8H196v16h16a8,8,0,0,1,0,16H196v16a8,8,0,0,1-16,0V152a8,8,0,0,1,8-8Zm-96,0a8,8,0,0,1,8,8v48a8,8,0,0,1-16,0V176H100v32a8,8,0,0,1-16,0V152a8,8,0,0,1,8-8Zm8,24v-8H100v8Zm-48-24a28,28,0,0,1,28,28v20a8,8,0,0,1-16,0V180a12,12,0,0,0-12-12H84v40a8,8,0,0,1-16,0V152a8,8,0,0,1,8-8Zm0,24a12,12,0,0,0,0-12ZM48,112h8V208a16,16,0,0,0,16,16H192a16,16,0,0,0,16-16V64h8a8,8,0,0,0,0-16H176V40a24,24,0,0,0-24-24H104A24,24,0,0,0,80,40v8H48a8,8,0,0,0,0,16v48Zm56-72a8,8,0,0,1,8-8h48a8,8,0,0,1,8,8v8H104ZM24,80a8,8,0,0,1,8-8H224a8,8,0,0,1,0,16H32A8,8,0,0,1,24,80Z">
                                        </path>
                                    </svg>
                                    <svg v-else-if="getFileType(file.key) === 'doc'" xmlns="http://www.w3.org/2000/svg"
                                        width="64" height="64" viewBox="0 0 256 256" fill="#60a5fa">
                                        <path
                                            d="M213.66,82.34l-56-56A8,8,0,0,0,152,24H56A16,16,0,0,0,40,40V216a16,16,0,0,0,16,16H200a16,16,0,0,0,16-16V88A8,8,0,0,0,213.66,82.34ZM160,51.31,188.69,80H160ZM200,216H56V40h96V88a8,8,0,0,0,8,8h48V216Zm-32-80a8,8,0,0,1-8,8H96a8,8,0,0,1,0-16h64A8,8,0,0,1,168,136Zm0,32a8,8,0,0,1-8,8H96a8,8,0,0,1,0-16h64A8,8,0,0,1,168,168Z">
                                        </path>
                                    </svg>
        ...
        <Dialog :open="isViewModalOpen" @close="isViewModalOpen = false" class="relative z-50">
            ...
            <div class="fixed inset-0 flex items-center justify-center p-4">
                <DialogPanel v-if="viewingDocument"
                    class="w-full max-w-2xl max-h-[80vh] overflow-y-auto rounded-2xl bg-zinc-900 border border-white/10 p-6 shadow-xl">
