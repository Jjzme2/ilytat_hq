<template>
    <div class="space-y-6 md:space-y-8 animate-fade-in">
        ...
        <div class="space-y-3">
            ...
            <div v-else-if="items.length === 0" class="text-center py-8 text-zinc-500 bg-zinc-900/40 rounded-xl border border-white/5">
                No upcoming events found.
            </div>
            <div v-else v-for="(item, index) in limitedItems" :key="index" class="bg-zinc-900/40 border border-white/5 rounded-xl p-4 flex gap-4 items-start hover:border-white/10 transition-colors group">
                <!-- Date column -->
                <div class="flex flex-col items-center min-w-[3rem] pt-1">
                    <span class="text-xs font-bold text-zinc-500 uppercase">{{ new Date(item._date).toLocaleString('default', { month: 'short' }) }}</span>
                ...
                <div class="flex-1">
                    ...
                    <p v-if="item.description" class="text-xs text-zinc-400 mt-1 line-clamp-1">{{ item.description }}</p>
                ...
                <!-- Delete Action -->
                <button 
                    v-if="item._type === 'event'" 
                    @click="handleDeleteEvent(item.id)"
