...
## 2026-02-12 - Reactive State Batch Updates
**Learning:** Registering commands one by one in a loop (`forEach -> commands.value.push`) triggers Vue's reactivity system N times, causing unnecessary overhead during initialization.
**Action:** Implement and use batch registration methods (e.g., `registerCommands`) that push all items to the reactive array in a single operation to trigger reactivity only once.
